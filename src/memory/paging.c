...
struct page_directory_handle *page_directory_init_4gb(const uint8_t flags)
{
    ...
    chunk->directory = kzalloc(sizeof(uintptr_t) * PAGING_ENTRIES_PER_TABLE);
    for (int i = 0; i < PAGING_ENTRIES_PER_TABLE; ++i) {
        ...
        chunk->directory[i] = (uintptr_t)page_table_entry | flags | PAGING_WRITABLE_PAGE;
...
int paging_v_addr_to_paging_indexes(const void *const virtual_addr, uintptr_t *const directory_index_out, uintptr_t *const table_index_out)
{
    if (!paging_addr_is_page_aligned(virtual_addr)) {
        return -EINVAL;
    }

    *directory_index_out = (uintptr_t)virtual_addr / (PAGING_ENTRIES_PER_TABLE * PAGING_PAGE_SIZE);
    const uintptr_t offset = (uintptr_t)virtual_addr % (PAGING_ENTRIES_PER_TABLE * PAGING_PAGE_SIZE);
    *table_index_out = offset / PAGING_PAGE_SIZE;

    return 0;
}

int paging_map_v_addr(struct page_directory_handle *const chunk, const void *const virtual_addr, const void *const page_ptr,
                      const uint8_t flags)
{
    ...
    const uintptr_t page_directory_entry = chunk->directory[directory_index];
